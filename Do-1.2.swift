                ...
                block {
                    barrierAsync(synchQueue) {
                        if --token.executingCount < token.limit && token.queuedOperations.count > 0 {
                            ...
                            concurrent(token, operation.0, operation.1)
                        }
                    }
                }
            }
...
public class ConcurrentToken {
    ...
    private var queuedOperations = [(dispatch_queue_t, ConcurrentBlock)]()
...
public func after(_ seconds: Double, _ queue: dispatch_queue_t = mainQueue, _ block: dispatch_block_t) {
...
/**
    ...
    Usage:
        ...
        cancel()
*/
public func afterCancel(_ seconds: Double, _ queue: dispatch_queue_t = mainQueue, _ block: dispatch_block_t) -> () -> Void {
    var cancelled = false
